...
describe('RealBackendDataSource', () => {
  ...
  it('fetches pipeline data with auth header', async () => {
    ...
    const ds = new RealBackendDataSource({ baseUrl: 'https://api.example.com', authToken: 'token123' });
  ...
  it('returns null on 404', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, status: 404, text: async () => '', statusText: 'Not Found' }) as any);
  ...
  it('throws on non-404 error', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, status: 500, text: async () => 'boom', statusText: 'Server Error' }) as any);
