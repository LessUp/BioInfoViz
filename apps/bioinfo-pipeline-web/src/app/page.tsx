...
type CategoryIcon = typeof Dna

const CATEGORY_ICONS: Partial<Record<string, CategoryIcon>> = {
  germline: Dna,
    ...
    <div className="relative min-h-screen">
      ...
      >
        ...
        <motion.section
          ...
          className="grid gap-16 lg:grid-cols-[1.2fr_1fr] lg:items-center"
        >
          <div className="max-w-2xl">
            ...
            <p className="mt-8 text-lg leading-relaxed text-zinc-600 dark:text-zinc-300">
              我们提供一站式的可视化教学与演示平台。从 BWA
              比对算法的微观细节，到全基因组分析的宏大流程，通过交互式动画与实时仪表盘，让复杂的生物信息学变得触手可及。
            </p>
          ...
          <div className="relative hidden lg:block">
            <Card
              elevation="lg"
              className="relative z-10 overflow-hidden border-zinc-200/50 bg-white/80 p-8 backdrop-blur-xl dark:border-zinc-700/50 dark:bg-zinc-900/80"
            >
              ...
              <ul className="space-y-6">
                {[
                  {
                    title: '全流程可视化',
                    desc: '覆盖 WES、RNA-Seq、单细胞等主流分析流程',
                    icon: FlaskConical,
                    color: 'text-blue-500',
                  },
                  {
                    title: '算法原理动画',
                    desc: '深入浅出展示比对、组装等核心算法机制',
                    icon: Dna,
                    color: 'text-emerald-500',
                  },
                  {
                    title: '交互式学习',
                    desc: '循序渐进的任务系统，记录你的每一步成长',
                    icon: GraduationCap,
                  ...
                  <li key={i} className="flex gap-4">
                    <div
                      className={cn(
                        'mt-1 flex h-8 w-8 flex-none items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800',
                        item.color
                      )}
                    >
                ...
                >
                  <div>
                    <div className="flex items-start justify-between">
                      ...
                      <Badge tone="neutral" className="bg-zinc-100 dark:bg-zinc-800">
                        {DIFFICULTY_LABEL[pipeline.difficulty] ?? pipeline.difficulty}
                      </Badge>
                    </div>
        ...
        <motion.section
          id="learning-paths"
          variants={itemVariants}
          className="space-y-10 rounded-3xl bg-white/50 p-8 shadow-sm ring-1 ring-zinc-200/50 backdrop-blur-md lg:p-12 dark:bg-zinc-900/30 dark:ring-zinc-800/50"
        >
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <motion.section variants={itemVariants} className="space-y-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">知识库与讲义</h2>
            </div>
          ...
          <motion.section variants={itemVariants} className="space-y-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">独立实验工具</h2>
            </div>
              ...
              {labTools.map((tool) => (
                <Link key={tool.name} href={tool.href} className="block h-full group">
                  <Card
                    interactive
                    elevation="sm"
                    className="h-full border-zinc-200 bg-white/60 transition-all hover:border-purple-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900/60 dark:hover:border-purple-700"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <Badge
                        tone="neutral"
                        className="group-hover:bg-purple-100 group-hover:text-purple-700 dark:group-hover:bg-purple-900/30 dark:group-hover:text-purple-300 transition-colors"
                      >
                        {tool.tag}
                      </Badge>
